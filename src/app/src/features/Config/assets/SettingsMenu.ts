				...
				settings: [
					...
					},
					{
						...
						disabled: () => {
							...
							const c_settings = get(controller, "settings.settings", {});
					...
					},
					{
						...
						description: "Show a carving summary at the end of each job.",
						type: "boolean",
					},
					{
						...
						onUpdate: () => {
							...
							if (!spindleFunctions && mode === LASER_MODE) {
								const preferredUnits =
									store.get("workspace.units") === IMPERIAL_UNITS
										? "G20"
										: "G21";
								...
								const getSpindleOffsetCode = (
									...
									const [xoffsetAdjusted, yOffsetAdjusted] =
										calculateAdjustedOffsets(xOffset, yOffset, preferredUnits);
		...
		settings: [
			{
				...
				settings: [
					...
					},
					{
						...
						hidden: (getPending) => {
							const strategy = getPending("workspace.toolChangeOption", "");
		...
		settings: [
			{
				...
				settings: [
					...
					},
					{
						...
						description: "Periodically announce job completion percentage.",
						type: "boolean",
						onUpdate: () => {
							pubsub.publish("accessibility:update");
						},
					},
					{
						...
						unit: "%",
						hidden: () =>
							!store.get("workspace.accessibility.jobProgressAnnouncements"),
						onUpdate: () => {
							pubsub.publish("accessibility:update");
						},
					},
				...
				settings: [
					{
						...
						description: "Play short sounds for specific machine events.",
						type: "boolean",
						onUpdate: () => {
							pubsub.publish("accessibility:update");
						},
					...
					},
					{
						...
						description: "Play sound when the machine enters an alarm state.",
						type: "boolean",
						hidden: () =>
							!store.get("workspace.accessibility.audioCues.enabled"),
						onUpdate: () => {
			...
			},
			{
				...
				settings: [
					...
					},
					{
						...
						type: "boolean",
						hidden: () =>
							!store.get("workspace.accessibility.gcodeSummary.enabled"),
						onUpdate: () => {
							pubsub.publish("accessibility:update");
						},
					},
				...
				settings: [
					{
						...
						description: "Show an overlay with active keyboard shortcuts.",
						type: "boolean",
						onUpdate: () => {
							pubsub.publish("accessibility:update");
						},
