...
const appMain = () => {
	...
	if (fs.existsSync(faviconPath)) {
		...
		log.warn(
			`Favicon not found at ${faviconPath}, skipping favicon middleware`,
		);
	}
...

	{
		...
		app.get(
			urljoin(settings.route, "api/alarmList/recent"),
			api.alarmList.fetchRecent,
		);
		app.delete(
			urljoin(settings.route, "api/alarmList"),
			api.alarmList.clearAll,
		);
		...
		// Release Notes
		app.get(
			urljoin(settings.route, "api/releasenotes"),
			api.releaseNotes.fetchReleaseNotes,
		);
